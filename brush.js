...
d3.chart.brush = function() {
  ...
  function chart(container) {
    ...
    var extent = d3.extent(data, function(d) {
      return new Date(d.properties.article_date).getTime();
    })
    ...
    rects.attr({
      x: function(d) { return scale(new Date(d.properties.article_date));},
      y: 0,
      width: 1,
      height: height
    ...
    brush.on("brushend", function() {
      ...
      var filtered = data.filter(function(d) {
        return (new Date(d.properties.article_date) > ext[0] && new Date( d.properties.article_date) < ext[1])
      })
  ...
  chart.highlight = function(data) {
    ...
    rects.data(data, function(d) { return d.properties.id })
    .style("stroke", "orange")
